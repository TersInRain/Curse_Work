...
def catalog(request):
    ...
    class Product_tmp:
        def __init__(self, product, images):
            ...
            self.productSroke = product.productSroke
            self.productImages = []
            for img in images:
...
def product(request, parametr):
    ...
    class Product_tmp:
        def __init__(self, product, images):
            ...
            self.productDescriptionFull = product.productDescriptionFull
            self.productPrice = product.productPrice
            self.sroke = product.productSroke 
            self.productImages = []
            for img in images:
                self.productImages.append(str(img.img.url))
            self.productImage = self.productImages[0]
...
def approveOrder(request, parametr):
    ...
    status = Status.objects.get(text = 'Готово')
...
def rejectOrder(request, parametr):
    orders = Order.objects.filter(order_id = parametr)
    status = Status.objects.get(text = 'Отказано')
...
def razrabOrder(request, parametr):
    orders = Order.objects.filter(order_id = parametr)
    status = Status.objects.get(text = 'В разработке')
...
def resetOrder(request, parametr):
    orders = Order.objects.filter(order_id = parametr)
    status = Status.objects.get(text = 'Консультация')
